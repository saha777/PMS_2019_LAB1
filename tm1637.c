...
//--
void TM1637_display_custom(uint8_t Seg_N,uint8_t DispData)//  ? ???????????? ?????  (?? 0 ?? 3)
{
  int8_t SegData;
  SegData = DispData;
  TM1637_Generate_START();                                
	TM1637_WriteData(ADDR_FIXED);//1:????????????? ??????
...
//--
void TM1637_display(uint8_t Seg_N,int8_t DispData)//  ? ???????????? ?????  (?? 0 ?? 3)
{
  int8_t SegData;
...
void TM1637_display_all(uint16_t Digit) 											
{
  ...
  TM1637_Generate_STOP();
  TM1637_Generate_START();
  TM1637_WriteData(Cmd_SetAddr);//????? ??????? ???????? 0xc0
  for(i=0;i < 4;i ++)
  {
